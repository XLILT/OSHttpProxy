...
def request_os_server(headers, body):
    #host = headers.get('Host', '')
    #host_ip, _, port = host.rpartition(':')
    host_ip = "xl.forever.love"
    port = 80
    ...
    response_wrap = ResponseWrap()
   
    conn = httplib.HTTPConnection("%s:%d" %(host_ip, port))

    start_ts = time.time()
    conn.request("POST", "/_gh", body, headers)

    xlog.debug("request cost %d", time.time() - start_ts)

    print "0"
    response = conn.getresponse()
    print "1"
    response_wrap.status = response.status
    response_wrap.reason = response.reason
    response_wrap.headers = dict(response.getheaders())
    
    response_wrap.data = response.read()
    print "2"
    conn.close()
    print "3"
    #print response_wrap.data

    #content, status, response = http_client.request('POST', '_gh', headers, body)
    print "request_os_server done"
...
def request_os_proxy(method, url, headers, body):
    # make retry and time out
    time_request = time.time()
    request_headers, request_body = pack_request(method, url, headers, body)
    error_msg = []
    ...
    while True:
        ...
        try:
            #response = request_gae_server(request_headers, request_body)
            response = request_os_server(request_headers, request_body)
...
def handler(method, url, headers, body, wfile):
    ...
    try:
        ...
        response = request_os_proxy(method, url, headers, body)        
    except GAE_Exception as e:
